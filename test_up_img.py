...
class distribution_Gaussian(Model):
    ...
    def initFromLabelledDatas(self):
        ...
        self.mus = ndatas.reshape(n_runs, n_sem+n_shot + 3 + n_queries, n_ways, n_nfeat)[:, :n_sem+n_shot+3, ].mean(1)
...
if __name__ == '__main__':
    ...
    alpha = 0.75
    import FSLTask
    cfg = {'sem': n_sem, 'shot': n_shot, 'ways': n_ways, 'queries': n_queries}
    FSLTask.loadDataSet("Aluminum")
    ...
    labels = torch.arange(n_ways).view(1, 1, n_ways).expand(n_runs, n_sem + n_shot+ 3 + n_queries, 5).clone().view(n_runs,n_samples)  # [1000,80]
    # print('labels', labels)
    beta = 0.5
